...
const (
	driverStateHAND          = 0
	driverStatePICKUP        = 1
	driverStateONGOING       = 2
	passengerStateHAND       = 0
	passengerStateWAITCOMPET = 1
	passengerStateWAITPICKUP = 2
	passengerStateONGOING    = 3
	orderStateINVALID        = 0
	orderStateWAITCOMPET     = 1
	orderStateONGOING        = 2
	orderStateFINISH         = 3
	orderStateABORT          = 4
)
...
// Order is the internal struct for "order" in taxing
// which store the order's specific information
type Order struct {
	ID         uint64  `json:"id"`
...
// OrderPool is the internal struct for "order pool"
// which store all the orders to be assigned
...
// User is the internal struct for "user"
// which store user's personal specific information
type User struct {
...
// args: userName pwdHash s_lng s_lat d_lng d_lat timeNow s_PlaceName d_PlaceName
// s means start point, d means desctination point
func (c *Chaincode) passengerSubmitOrder(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if passenger.PassengerState != passengerStateHAND {
	...
	newOrder.State = orderStateWAITCOMPET
	...
	passenger.PassengerState = passengerStateWAITCOMPET
...
func (c *Chaincode) driverCompetOrder(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	if driver.DriverState != driverStateHAND {
	...
	if orderState != orderStateWAITCOMPET {
	...
	driver.DriverState = driverStatePICKUP
	...
	err = c.setPassengerState(stub, order.Passenger, passengerStateWAITPICKUP)
	if err != nil {
		return nil, err
	}
...
func (c *Chaincode) driverPickUp(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	driver.DriverState = driverStateONGOING
	err = c.setUser(stub, args[0], driver)
	if err != nil {
		return nil, err
	}
	err = c.setPassengerState(stub, order.Passenger, passengerStateONGOING)
	if err != nil {
		return nil, err
	}
...
func (c *Chaincode) driverFinishOrder(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	driver.DriverState = driverStateHAND
	err = c.setUser(stub, args[0], driver)
	if err != nil {
		return nil, err
	}
	passenger.PassengerState = passengerStateHAND
...
// RetOrder is the json strcuct for order pool's return value
...
//=================================================================================================================================//
//	private function for date load/save
//=================================================================================================================================//
...
// RetBalance is the json struct for return value of banlance
